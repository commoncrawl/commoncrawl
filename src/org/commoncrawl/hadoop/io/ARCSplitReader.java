...
public class ARCSplitReader implements RecordReader<Text, ArcFileItem> {
     ...
     *
     * @param split     the {@link ARCSplit} to read
     * @param source    the {@link ARCSource} to open resources with
     * @param blockSize the number of bytes at a time to read from each input
     *                  stream
     */
        ...
        public void run() {
            ...
            for (int i = 0; i < readers.length; i++) {
                ...
                while (true) {
                    ...
                    try {
                        stream = source.getStream(resource.getName(),
                            streamPosition, lastError, failures);
                        if (stream == null) {
     ...
     * 
     * @param  item the {@link ArcFileItem} to populate
     *
     * @return <tt>true</tt> if a new item was read, or <tt>false</tt> if there
     *         are no more 
     * 
     * @throws IOException if an IO error or timeout occurs 
     */
    ...
    public boolean next(Text key, ArcFileItem value) throws IOException {
        ...
        if (next(value)) { 
          ...
          //TODO: we are going to clear the uri field's dirty flag since it is redundant
          //           this means the uri will not be serialized with the rest of the item's data...
