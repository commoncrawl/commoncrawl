...
/**
 * An {@link ARCSource} for gzipped ARC files stored on Amazon S3 that uses
 * <a href="http://jets3t.s3.amazonaws.com/index.html">JetS3t</a> to interact
 * with S3.
 *
 * @author Albert Chern
 */
public class JetS3tARCSource extends ARCSplitCalculator
    implements ARCSource, JobConfigurable {
    
    /**
     * <tt>jets3t.arc.source.input.prefixes.csv</tt> - the property where the
     * prefixes to match for input files are stored (in the future, we may want
     * to extend this to do some sort of globbing or regular expression
     * matching).
     *
     * @see #setInputPrefixes
     */
    public static final String P_INPUT_PREFIXES =
        "jets3t.arc.source.input.prefixes.csv";
     ...
     */
    public static final String P_AWS_ACCESS_KEY_ID =
        "jets3t.arc.source.aws.access.key.id";
     ...
     */
    public static final String P_AWS_SECRET_ACCESS_KEY =
        "jets3t.arc.source.aws.secret.access.key";
    
    /**
     * <tt>jets3t.arc.source.bucket.name</tt> - the property where the name of
     * the S3 bucket to access is stored.
     *
     * @see #setBucketName
     */
    public static final String P_BUCKET_NAME =
        "jets3t.arc.source.bucket.name";
     ...
     * processing.
     * 
     * <p> Call this method to set up the input parameters for a job.  The
     * matching will be a pure prefix query with no delimiters.
     * 
     * @param job      the job to set the prefix for
     * @param prefixes the key prefixes to match S3 objects with
     *
     ...
     *
     * @param job the job to get the input prefixes from
     *
     * @see #P_INPUT_PREFIXES
     *
    ...
    public static String[] getInputPrefixes(JobConf job) {
        ...
        return inputPrefixes == null? null :
            EscapeUtils.split(',', inputPrefixes);
    }
    
    /**
     * Sets the AWS access key ID of the S3 account to use.
     *
     * @param job            the job to set the AWS access key ID for
     ...
     * Sets the AWS access key ID of the S3 account to use.
     *
     * @param job                the job to set the AWS secret access key for
     ...
     *
     * @param job        the job to set the bucket name for
     ...
     * <p> Default is 4.
     * 
     * @param job      the job to set the maximum number of retries for
     * @param maxTries the maximum number of attempts per file
     */
    ...
    protected void configureImpl(JobConf job) {
        try {
            ...
            AWSCredentials awsCredentials =
                new AWSCredentials(awsAccessKeyId, awsSecretAccessKey);
    ...
    public InputStream getStream(String resource, long streamPosition,
        ...
        if (lastError == null || previousFailures < maxTries) {
            ...
            LOG.info("Opening " + resource + " at byte position " +
                streamPosition + ", attempt " + (previousFailures + 1) +
                " out of " + maxTries);
            S3Object object = service.getObject(bucket, resource, null, null,
                null, null, streamPosition, null);
