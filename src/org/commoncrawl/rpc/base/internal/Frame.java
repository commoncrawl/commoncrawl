...
public class Frame {
  ...
  public static final Log LOG = LogFactory.getLog(Frame.class);  
	...
	static final int  REQUEST_ESSENTIAL_FIELDS_MASK = (1 << MSG_HEADER_FIELD_TYPE) | (1 << MSG_HEADER_FIELD_SERVICE) | (1 << MSG_HEADER_FIELD_METHOD) | 
	...
	public static final class Encoder { 
		...
		private int encode(HeaderOutputStream header,PayloadOutputStream payload)throws IOException { 
		...
		private IncomingFrame getNextFrame(boolean isRequestFrame) throws IOException { 
			...
			try { 
				...
				if (_lookAheadSize == FRAME_HEADER_SIZE) {
					...
					// if there is enough data in the input stream to read the entire msg (header + payload)
					if (_stream.available() >= _headerSize + _payloadSize) { 
						...
						int requiredFieldMask = (isRequestFrame == true) ? REQUEST_ESSENTIAL_FIELDS_MASK : RESPONSE_ESSENTIAL_FIELDS_MASK;
						
						if ((fieldMask & requiredFieldMask) != requiredFieldMask) { 
							throw new IOException("Invalid Frame Header Detected");
						}
						
						// otherwise good to go ... create payload stream and return populate frame 
				...
				if (_lookAheadSize == FRAME_HEADER_SIZE) { 
					if (_lookAheadBuffer[0] != SIGNATURE[0] || _lookAheadBuffer[1] != SIGNATURE[1] || 
						...
						debugBuffer.append("  Got:[" + _lookAheadBuffer[0]+ "][" + _lookAheadBuffer[1]+ "][" + _lookAheadBuffer[2]+ "][" + _lookAheadBuffer[3]+ "]\n");
					...
					LOG.error("getNextFrame Threw Exception:" + CCStringUtils.stringifyException(e));
					LOG.error("Details:");
	...
	public void testEncoderDecoder() throws Exception {
		...
		/** UnitTestStruct1 based on rpc defiinition: 
		...
		* */
		...
		AsyncRequest<UnitTestStruct1,UnitTestStruct1> request = new AsyncRequest<UnitTestStruct1,UnitTestStruct1>("testService", "testMethod", inputStruct, outputStruct, null );
		...
		while (output.available() != 0 ) {
			...
			// validate that the decoder doesn't return the frame until the appropriate time 
