...
public class MimeTypeFilter {
  ...
  private final static Pattern TRUNCATION_REGEX = Pattern.compile("^([^a-zA-Z0-9]*)([a-zA-Z0-9-/\\.+]*).*");
  private final static Pattern APPLICATION_TYPE_REGEX = Pattern.compile("^application/(.*)");
...
//      "flv",
  ...
  public static boolean checkForExclusion(String mimeType) { 
    ...
    if (truncatedMimeType != NO_TYPE_MIMETYPE) { 
        if (invalidExtensionMatcher.matches(truncatedMimeType)) { 
          ...
          if (invalidBinaryTypesMatcher.matches(truncatedMimeType) || excludedMimeTypesMatcher.matches(truncatedMimeType)) { 
            return true;
          }
          Matcher appTypeMatcher = APPLICATION_TYPE_REGEX.matcher(truncatedMimeType);
          
          if (appTypeMatcher.matches() && !validAppTypesMatcher.matches(appTypeMatcher.group(1))) {
            return true;
          }
        }
    }
  ...
  public static boolean isTextType(String contentTypeStr) { 
      ...
      if (truncatedMimeType.startsWith("text") && (truncatedMimeType.length() == 4 || truncatedMimeType.charAt(4) == '/')) {
      	return true;
      }
      else { 
  ...
  private static String testStrings[] = { 
    ...
    "text/html; charset=utf8_decode",
    "text/html; charset=windows-31J",
    "text/html;CHARSET=x-euc-jp",
    "text/html;charset=EUC_CN",
    "text/html;charset=ks_c_5601-1987",
    "text/javascript; Charset=windows-1252",
    ...
    ContentTypeAndCharset metadata = new ContentTypeAndCharset();
  	...
  	for (String typeStr : testStrings) {
	      ...
	      if (checkForExclusion(metadata._contentType))
	        System.out.println("Excluded:" + metadata._contentType + " RAW:" + typeStr);
	      else 
	      	System.out.println("Passed:" + metadata._contentType + " RAW:" + typeStr + " IsValidTextType:" + isValidTextType(metadata._contentType));
  		}
    }
  }
