...
public class BandwidthUtils {
  ...
  public  static class BandwidthHistory  {
    ...
    private static String byte_units[] = { "B/s", "KB/s", "MB/s", "GB/s" };
    private static String bit_units[] = { "b/s", "Kb/s", "Mb/s", "Gb/s" };
    ...
    public void calcSpeed(BandwidthStats statsOut) { 
      ...
      if (downloadAmount !=0 && downloadTime != 0) { 
        ...
        if (statsOut.bytesPerSecond < 1024.0) { 
          ...
          statsOut.scaledBytesPerSecond = statsOut.bytesPerSecond / (1024.0 * 1024.0);
          statsOut.scaledBytesUnits         = byte_units[2];
        }
        else  { 
          statsOut.scaledBytesPerSecond = statsOut.bytesPerSecond / (1024.0 * 1024.0 * 1024.0);
        ...
        if (statsOut.bitsPerSecond < 1024.0) { 
          ...
          statsOut.scaledBitsPerSecond = statsOut.bitsPerSecond / (1024.0 * 1024.0);
          statsOut.scaledBitsUnits         = bit_units[2];
        }
        else  { 
          statsOut.scaledBitsPerSecond = statsOut.bitsPerSecond / (1024.0 * 1024.0 * 1024.0);
  ...
  public static class RateLimiter { 
    ...
    /** checkRateLimit 
     *  
     *   take in bytes available and based on desired rate limit return
     *   adjusted bytes available 
     * 
     * @param bytesAvilable
     * @return
     */
    public int checkRateLimit(int byteAvailable) { 
      ...
      // if first time into call or if delta between last call and current window 
      ...
      return Math.min((_desiredBandwidthBytes - _bytesAccumulated),byteAvailable);
    }
