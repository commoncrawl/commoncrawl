 ...
 *
 */
public final class ArcFileReader extends InflaterInputStream implements NIODataSink  {
 ...
 /**
 * Costructs a new ArcFileReader object with specified block size (for allocations) 
 */
   ...
   * This is a BLOCKING CALL - it will block on _consumerQueue.take if no data is available...  
  ...
  public static class ArcFileBuilder {
    ...
    /** Constructor 
     * 
     * @param itemToConstruct - the ArcFileItem to build
     */
    ...
    private final boolean checkForCRLFTerminator(byte matchingChar) { 
      if (matchingChar == '\n') {
        switch (matchCount) { 
          case 1: ++matchCount; break;
          case 3: matchCount =0;return true;
          default: matchCount = 0;
        }
      }
      else if (matchingChar == '\r') {
        switch (matchCount) {
          case 2: ++matchCount;break;
          default: matchCount=1;break;
        }
      }
      else { 
        matchCount = 0;
        ...
        switch (++tokenCount) { 
          ...
          case 3: { 
            ...
            try {
              ...
              LOG.error("Invalid Timestamp Encountered in Item Metdata. URL:"+_item.getUri() 
                  + " Timestamp:" + timeStamp 
                  + " Metadata:" + metadata);
    ...
    public final void finish() throws IOException { 
      if (_state == State.ReadingContent ) { 
        ...
        if (_buffer.getCount() == 0) { 
          LOG.error("ArcFileBuilder Encountered Item with Zero Length Content. URI:" + _item.getUri());
        }
        else { 
        ...
        throw new IOException("ArcBuilder finish calledin Invalid State. State:" + _state + " ArcFile:" + _item.getArcFileName() +  " Position:" + _item.getArcFilePos() +" Item URI:" + _item.getUri());
      }
    }
      ...
      while(_activeBuffer != null && _activeBuffer.remaining() != 0) {
        ...
        if (_state  != ArcFileBuilder.State.ReadingContent) { 
          ...
          if (available < _activeBuffer.remaining()) { 
            ...
            int growByAmount = Math.max(_activeBuffer.remaining() - available, _blockSize * 2);
            // and grow the buffer ... 
            _buffer.setCapacity(_buffer.getCapacity() + growByAmount);
          }
          // copy the buffer data in one go ...
          _buffer.append(_activeBuffer.array(),_activeBuffer.position() + _activeBuffer.arrayOffset(),_activeBuffer.remaining());
          _activeBuffer = null;
        }
      }
      ...
      public void run() {
        try { 
          ...
          while (hasMoreItems()) {
            ...
            LOG.info("GOT Item URL:" + item.getUri() + " StreamPos:" + item.getArcFilePos() + " Content Length:" + item.getContent().getCount());
            ...
            //LOG.info("Content:");
            /*
            ByteArrayInputStream inputStream = new ByteArrayInputStream(item.getContent().getReadOnlyBytes(),0,item.getContent().getCount());
            BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream,Charset.forName("ASCII")));
            String line = null;
            while ((line = reader.readLine()) != null) { 
              LOG.info(line);
            }
