...
public class URLUtils {
  ...
  public static String extractTLDName(String hostName) { 
    ...
    if (hostName.length() != 0) { 
        if (!invalidDomainCharactersRegEx.matcher(hostName).find()) { 
            ...
            if (parts.length >= 2) { 
                Collection<String> secondaryNames = TLDNamesCollection.getSecondaryNames(parts[parts.length - 1]);
                
                if (secondaryNames.size() != 0) {
                    // see if second to last part matches secondary names for this TLD or there is a wildcard expression for secondary name in rule set  
                    if (secondaryNames.contains(parts[parts.length - 2]) || secondaryNames.contains("*")) { 
                        ...
                        if (secondaryNames.contains("!" + parts[parts.length - 2])) { 
                            // in this case, second to last part is NOT part of secondary name
                    ...
                    // ok second to last part does not match set of known secondary names 
                    else { 
                        ...
                        if (secondaryNames.contains(extendedWildcard)) {
                            ...
                            if (parts.length >= 3) { 
                                // this implies that there must be four parts to the name to extract root
                                ...
                                if (secondaryNames.contains(exclusionRule2)) { 
                                    ...
                                    // ok extended wildcard matched. last 3 parts are part of the TLD 
                                    if (parts.length >= 4) { 
            ...
            if (parts.length >= 2) { 
                ...
                if (secondaryNames.size() != 0) {
                    ...
                    if (secondaryNames.contains(parts[parts.length - 2]) || secondaryNames.contains("*")) { 
                        ...
                        // check to see the part in not explicitly excluded ... 
                        if (secondaryNames.contains("!" + parts[parts.length - 2])) { 
                            // in this case, this is an explicit override. second to last part is NOT part of secondary name
                            return buildRootNameString(hostName, parts, parts.length - 2);
                        }
                        else { 
                        ...
                        if (secondaryNames.contains(extendedWildcard)) {
                            ...
                            if (parts.length >= 3) { 
                                ...
                                if (secondaryNames.contains(exclusionRule2)) { 
                                    ...
                                    if (parts.length >= 4) { 
                                        return buildRootNameString(hostName,parts,parts.length - 4);
                                    }
                                }
                            }
                        }
   ...
   * 
   * @param incomingURL
   * @param stripLeadingWWW - set to true to string www. prefix from the domain if present 
   * @return a canonical representation of the passed in URL that can be safely used as a replacement
